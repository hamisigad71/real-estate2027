      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-3">
            ...
            {results.statutoryFees && (
              <div className="pt-3 mt-3 border-t border-slate-100 space-y-2">
                <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Statutory Fees & Taxes</p>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-slate-600">Stamp Duty (4%)</span>
                  <span className="text-slate-900">{formatCurrency(results.statutoryFees.stampDuty, project.budgetRange.currency)}</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-slate-600">VAT (16%)</span>
                  <span className="text-slate-900">{formatCurrency(results.statutoryFees.vatAmount, project.budgetRange.currency)}</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-slate-600">NEMA & NCA Fees</span>
                  <span className="text-slate-900">{formatCurrency(results.statutoryFees.nemaFee + results.statutoryFees.ncaFee, project.budgetRange.currency)}</span>
                </div>
                <div className="flex items-center justify-between font-semibold text-slate-900 border-t border-slate-100 pt-1">
                  <span>Total Fees</span>
                  <span>{formatCurrency(results.statutoryFees.totalFees, project.budgetRange.currency)}</span>
                </div>
              </div>
            )}
          </div>
